...
extern pid32 pidDisplay;
extern pid32 pidGalaga;
...
int galaga(void)
{
	...
	while (1)
	{
		...
		for (int a = 0; a < 9; a++)
		{
			...
			{
				drawRect(easyEnemies[a].enemyX, easyEnemies[a].enemyY, 20, 20, BLACK);//Borra el enemigo
				easyEnemies[a].enemyY = 0;
				endGame();
				//**
			}
		...
		for (int i = 0; i < N_SHOOTS; i++)
		{
			...
			if (shoots[i] != 0)
			{
				...
				for (int j = 0; j < 9; j++)
				{
					...
					{
						if (!coll){
							send(pidDisplay, 2); // Aumenta puntaje
							coll = 1;
						}
		...
		for (int a = 0; a < 9; a++)
		{
			...
			{
				drawRect(hardEnemies[a].enemyX, hardEnemies[a].enemyY, 20, 20, BLACK);//Borra el enemigo
