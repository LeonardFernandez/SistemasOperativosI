...
void convertir(int in_fd, int out_fd, BMPHeader h, BMPInfoHeader infoh,int inicio, int fin,unsigned char *nueva_imagen) 
{
...
int main() 
{
    ...
    int porcion=height/3;
    int sobra=height%3;
    for(int i=0; i<3; i++){
        pid=fork();
        if(pid==0){ //Ejecuta hijo
            printf("Ejecuta hijo %d", pid);
            int inicio=i*porcion;
            int fin=(i+1)*porcion;
            convertir(in_fd,out_fd,h,infoh,0,1,nueva_imagen);
            exit(0);
        }else if(pid<0){
            printf("Error al crear hijo");
        }
    }
    //Esperamos que terminen los hijos
    for(int i=0; i<3; i++){
        wait(NULL);
    }
    //Padre escribe
